...
//Verify that user booked a fight ticket for round trip
...
public class RoundTripFlightBooking extends Base {
	...
	public void login() throws IOException, InterruptedException {
		...
		// String testLogin =driver.getTitle(); 
		 //softAssert.assertTrue(false,testLogin.equals("Login Successful"));
		/* String getActualTitle = driver.getTitle();
		 Boolean verifyTitle = driver.getTitle().equalsIgnoreCase("Login Successful");
		 softAssert.assertEquals(getActualTitle, "Login Successful");
		 softAssert.assertNotEquals(getActualTitle, "Login Successful");
		 softAssert.assertNull(verifyTitle);
		 softAssert.assertNotNull(verifyTitle);
		 softAssert.assertTrue("Login Successful".equals("Login Successful"), "First soft assert failed");
		 softAssert.assertFalse("Login Successful".equals("Login Successful"), "Second soft assert failed");*/
	...
	@Test(priority = 1, groups = { "FlightBooking" },description="Verify that user booked a fight ticket for round trip")
	public void FlightBookingroundtrip() throws InterruptedException {

		//ExtentTest	test= Report.extentRep.createTest("11_TC_FBM_03_Verify that user booked a fight ticket for round trip");
		JavascriptExecutor js = ((JavascriptExecutor) driver);
		...
		js.executeScript("document.getElementById('FlightsDateEnd').removeAttribute('readonly',0);");
		WebElement fromDateBox2 = driver.findElement(By.id("FlightsDateEnd"));
		fromDateBox2.clear();
		fromDateBox2.sendKeys(prop.getProperty("returndate"));
		fromDateBox2.sendKeys(Keys.TAB);
